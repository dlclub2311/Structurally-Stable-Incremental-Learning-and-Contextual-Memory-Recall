...
class ICarl(IncrementalLearner):
    ...
    def _training_step(self, train_loader, val_loader, initial_epoch, nb_epochs, record_bn=True, clipper=None):
        ...
        wait = 0
        self._n_batches = len(train_loader)
        ...
        for epoch in range(initial_epoch, nb_epochs):
            ...
            for i, input_dict in enumerate(prog_bar, start=1):
                ...
                loss.backward()
                #torch.nn.utils.clip_grad_norm_(training_network.parameters(), 10.0, error_if_nonfinite = False)
                self._optimizer.step()

                if clipper:
                    training_network.apply(clipper)

                self._print_metrics(prog_bar, epoch, nb_epochs, i, total_batches = len(train_loader))
    ...
    def _print_metrics(self, prog_bar, epoch, nb_epochs, nb_batches, total_batches):
        pretty_metrics = ", ".join(
        ...
        if nb_batches == total_batches:
                       logging.info("T{}/{}, E{}/{} => {}".format(self._task + 1, self._n_tasks, epoch + 1, nb_epochs, pretty_metrics))
    ...
    def compute_examplar_mean(feat_norm, feat_flip, indexes, nb_max):
        ...
        selected_d = D[..., indexes]  # original
        selected_d2 = D2[..., indexes] # original
