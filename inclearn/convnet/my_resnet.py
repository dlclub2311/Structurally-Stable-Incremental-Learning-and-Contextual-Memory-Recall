...
def gradcam_hook_functn(module, gradinput, gradoutput):
        module.gradcam_gradients = gradoutput[0]
...
class CifarResNet(nn.Module):
    ...
    def get_gradcam_gradients(self): 
        return self.stage_4_importance.gradcam_gradients

    def activate_gradcam_hooks(self): 
        print("\n\nRegistering Backward hook\n")
        self.gradcam_hook = self.stage_4_importance.register_full_backward_hook(gradcam_hook_functn)

    def deactivate_gradcam_hooks(self): 
        if self.gradcam_hook is not None:
            print("deactivating gradcam hooks")
            self.gradcam_hook.remove()
